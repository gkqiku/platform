...
<script lang="ts">
  ...
  import { ReduceContext, getClient, reduceCalls } from '@hcengineering/presentation'
  ...
  const updateCategory = reduceCalls(async function (ctx: ReduceContext, value: Status | undefined): Promise<void> {
    if (value === undefined) {
      category = undefined
      return
    }
    const r = await client.findOne(core.class.StatusCategory, { _id: value.category })
    if (!ctx.canceled) {
      category = r
    }
  })
