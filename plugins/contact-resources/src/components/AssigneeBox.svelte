...
<script lang="ts">
  ...
  import presentation, { ReduceContext, getClient, reduceCalls } from '@hcengineering/presentation'
  import {
    ActionIcon,
    Button,
    ButtonKind,
  ...
  const updateSelected = reduceCalls(async function (ctx: ReduceContext, value: Ref<Person> | null | undefined) {
    const r = value
